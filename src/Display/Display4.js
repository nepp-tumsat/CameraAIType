...
const Display = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const wordData = collection(db, 'word4');
  ...
  <div class="btn4">
      <div className="batsu4"><Link to="/" className="batsu1">×</Link></div>
  </div>
  ...
  <Slider 
    ...
    >
      {register.map((word) => (
        <div key={word.text} className="slider-content4">
          <img src={word.image} alt="画像が読み込めません"/>
          <p onClick={handleTextClick}></p>
          <div class="box-0204" onClick={handleTextClick}>
    ...
    <div style={{ textAlign: "center" }}>
        <br></br>
        <br></br>
          <button className="hidarisankaku4" onClick={previous}></button>
          <button className="migisankaku4" onClick={next}></button>
    </div>
    </>
  );
};
