...
const Display = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const wordData = collection(db, 'word6');
  ...
  <div class="btn6">
      <div className="batsu6"><Link to="/" className="batsu1">×</Link></div>
  </div>
  ...
  <Slider 
    ...
    >
      {register.map((word) => (
        <div key={word.text} className="slider-content6">
          <img src={word.image} alt="画像が読み込めません"/>
          <p onClick={handleTextClick}></p>
          <div class="box-0206" onClick={handleTextClick}>
    ...
    <div style={{ textAlign: "center" }}>
        <br></br>
        <br></br>
          <button className="hidarisankaku6" onClick={previous}></button>
          <button className="migisankaku6" onClick={next}></button>
    </div>
    </>
  );
};
